...
from src.schemas.school import SchoolCreate, SchoolUpdate, SchoolResponse
...
async def list_schools(
    ...
    if is_super_admin:
        ...
        school_ids = {role.school_id for role in user_roles if role.school_id is not None}
        ...
        count_query = select(func.count()).select_from(School).where(School.id.in_(school_ids))
        total_result = await db.execute(count_query)
        total = total_result.scalar_one()
        
        query = select(School).where(School.id.in_(school_ids)).offset((page - 1) * page_size).limit(page_size)
        result = await db.execute(query)
        schools = result.scalars().all()
...
async def get_school(
    ...
    has_access = is_super_admin or any(role.school_id == school_id for role in user_roles)
